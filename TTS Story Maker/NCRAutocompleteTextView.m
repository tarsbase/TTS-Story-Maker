...
// I changed these obviously-designed-for-light-appearance parameters
// since you know, Mojave's dark mode is a thing - Emily B.
...
#define POPOVER_BOLDFONT [NSFont systemFontOfSize: 13.0 weight: NSFontWeightBold]
#define POPOVER_TEXTCOLOR [NSColor controlTextColor]
...
- (void)drawSelectionInRect:(NSRect)dirtyRect {
	NSColor *colour = [NSColor systemBlueColor];

	// Support for Mojave's colour accents - Emily B.
...
- (void)awakeFromNib {
	...
	// Syntax Highlighter + Line Tally Controller - Emily B.
    ...
    // self.autocompletePopover.appearance = [NSAppearance appearanceNamed: NSAppearanceNameAqua];
...
- (NSArray *)completionsForPartialWordRange:(NSRange)charRange indexOfSelectedItem:(NSInteger *)index {
    if ([self.ncrDelegate respondsToSelector:@selector(textView:completions:forPartialWordRange:indexOfSelectedItem:)]) {
        return [self.ncrDelegate textView:self completions:@[] forPartialWordRange:charRange indexOfSelectedItem:index];
    }
    return @[];
}
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row {
    ...
    if (cellView == nil) {
        ...
        if ([self.ncrDelegate respondsToSelector:@selector(textView:imageForCompletion:)]) {
            NSImageView *imageView = [[NSImageView alloc] initWithFrame:NSZeroRect];
            [imageView setImageFrameStyle:NSImageFrameNone];
			
			// Just so we can at least see the character portraits better - Emily B.
    ...
    if ([self.ncrDelegate respondsToSelector:@selector(textView:imageForCompletion:)]) {
        NSImage *image = [self.ncrDelegate textView:self imageForCompletion:self.matches[row]];
...
// For compatibility with my syntax highlighter and line tally controller - Emily B.
- (NSDictionary<NSTextCheckingOptionKey, id> *)textView:(NSTextView *)view willCheckTextInRange:(NSRange)range options:(NSDictionary<NSTextCheckingOptionKey, id> *)options types:(NSTextCheckingTypes *)checkingTypes {
	...
	// make sure that our text processing functions get called
	if ([self.ncrDelegate respondsToSelector:@selector(textWillBeChecked)]) {
		[self.ncrDelegate textWillBeChecked];
	}
